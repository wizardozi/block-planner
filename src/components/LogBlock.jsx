...
export const LogBlock = ({
  ...
  const [tableData, setTableData] = useState([
    ['Timestamp', 'Date', 'Project', 'Category', 'Hours', 'Description'],
    ['', '', '', '', '', ''],
  ]);
  ...
  const handleAddColumn = () => {
    setTableData((prev) =>
      prev.map((row, rowIndex) => {
        const newCell = rowIndex === 0 ? `Column ${row.length + 1}` : '';
        return [...row, newCell];
      })
    );
  };
          ...
          <tr>
            {tableData[0].map((header, i) => (
              <th
                ...
                className="border border-gray-600 p-2 bg-gray-800 text-gray-100"
              >
                {header}
              </th>
            ))}
